...
# selected_features = data.columns[np.argsort(feature_importances)[::-1][:900]]  # 选择前900个重要特征
...
# 创建模型并使用最佳参数进行训练
model = AdaBoostClassifier(learning_rate=1,n_estimators=200)
model.fit(x_train, y_train)

# 保存模型
joblib.dump(model, 'EAAC-AB.joblib')
