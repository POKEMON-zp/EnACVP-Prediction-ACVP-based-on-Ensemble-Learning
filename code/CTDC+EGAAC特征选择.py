...
selected_features = data.columns[np.argsort(feature_importances)[::-1][:400]]  # 选择前20个重要特征
x_train = x_train[selected_features]
x_test = x_test[selected_features]
model = CatBoostClassifier(iterations=100, learning_rate=0.07, depth=8)
model.fit(x_train, y_train)
# joblib.dump(model, '(CTDC+EGAAC)-CB-S.joblib')
# 预测
