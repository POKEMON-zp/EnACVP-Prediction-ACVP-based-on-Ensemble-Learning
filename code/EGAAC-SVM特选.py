...
selected_features = data.columns[np.argsort(feature_importances)[::-1][:200]]  # 选择前20个重要特征
x_train = x_train[selected_features]
x_test = x_test[selected_features]
# x_test.to_csv("EGAAC_SVM_test.csv", index=False)
# y_test.to_csv("EGAAC_SVMy_test.csv", index=False)
# # 建立模型
...
# # 创建模型
model = svm.SVC(kernel='rbf',C=100,probability=True)
model.fit(x_train,y_train)
#保存模型

joblib.dump(model, 'EGAAC-SVM-S.joblib')
# 预测
