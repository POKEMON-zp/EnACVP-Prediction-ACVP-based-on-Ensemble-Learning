...
# x_test.to_csv("EAACx_test.csv", index=False)
# y_test.to_csv("EAACy_test.csv", index=False)
#使用smote过采样
...
#{'depth': 9, 'iterations': 90, 'learning_rate': 0.09999999999999998}
# 创建模型
model = CatBoostClassifier(iterations=500, learning_rate=0.11, depth=11)
model.fit(x_train, y_train)
# #保存模型
#
joblib.dump(model, 'EAAC-CB.joblib')
# 预测
