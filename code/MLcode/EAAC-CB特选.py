...
# np.save('EAAC_train_X_smote.npy', x_train)
# np.save('EAAC_train_Y_smote.npy', y_train)
# LightGBM特征选择
lgb_clf = lgb.LGBMClassifier(random_state=42)
lgb_clf.fit(x_train, y_train)
feature_importances = lgb_clf.feature_importances_
selected_features = data.columns[np.argsort(feature_importances)[::-1][:700]]  # 选择前20个重要特征
...
# # 创建模型
model = CatBoostClassifier(iterations=500, learning_rate=0.11, depth=11)
model.fit(x_train, y_train)
# # #保存模型
# joblib.dump(model, 'EAAC-CB-S.joblib')
# # 预测
